...
export default async function NewBudgetItemPage({
  ...
  const { data: trip } = await supabase
    .from("trips")
    .select("id, slug, title, budget_currency")
    .eq("slug", id)
    .maybeSingle();
        ...
        <form action={createBudgetItem}>
          ...
          <div className="rounded-xl p-8" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
            ...
            <div className="mb-5">
              ...
              <input id="bi-label" name="label" type="text" required placeholder="z. B. Visagebühren Oman" style={FIELD_STYLE} />
            </div>

            <div className="mb-5">
              <label htmlFor="bi-category" style={LABEL_STYLE}>Kategorie</label>
              <select id="bi-category" name="category" defaultValue={BUDGET_CATEGORY_ORDER[0]} style={FIELD_STYLE}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              <div>
                ...
                <input id="bi-amount" name="amount" type="text" inputMode="decimal" placeholder="z. B. 120.00" style={FIELD_STYLE} />
              </div>
              <div>
                <label htmlFor="bi-currency" style={LABEL_STYLE}>Währung</label>
                <input id="bi-currency" name="currency" type="text" defaultValue={trip.budget_currency} style={FIELD_STYLE} />
              </div>
            </div>
            ...
            {(stages ?? []).length > 0 && (
              <div className="mb-8">
                ...
                <select id="bi-stage" name="stage_id" defaultValue="" style={FIELD_STYLE}>
