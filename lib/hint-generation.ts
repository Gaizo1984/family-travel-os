...
export async function generateAllTripHints(): Promise<HintGenerationResult> {
  ...
  const { data: tripsRaw } = await supabase
    .from('trips')
    .select('id, slug, family_id, status, start_date, end_date')
    .neq('status', 'archived')

  const trips = (tripsRaw ?? []).filter((t) => !isTripHistorical(t))

  for (const trip of trips) {
    const drafts = await generateHintsForTrip(supabase, trip.id, trip.family_id, trip.slug)
    await upsertHintsForTrip(supabase, trip.id, trip.family_id, drafts, result)
...
async function upsertHintsForTrip(
  ...
  for (const draft of drafts) {
    ...
    if (!existing) {
      await supabase.from('trip_hints').insert({
        family_id: familyId, trip_id: tripId,
    ...
    await supabase.from('trip_hints').update({
  ...
  if (staleIds.length > 0) {
    await supabase.from('trip_hints').delete().in('id', staleIds)
